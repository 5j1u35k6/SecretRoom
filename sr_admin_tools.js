...
function buildRecoveryMessage(code, expiresAtMs) {
  return [`你的 SecretRoom 臨時密碼：${code}`, '', '10 分鐘內有效。', `到期時間：${fmtTime(expiresAtMs)}`, '', '請用會員帳號和這組臨時密碼登入。', '登入後記得立刻換成自己的密碼。', '', '不是你申請的話，請聯絡管理員。'].join('\n');
}
async function sendRecoveryMail(user, uid, code, expiresAtMs) {
  if (!window.emailjs) throw new Error('Email 功能還沒準備好');
  if (!user.email) throw new Error('這位會員沒有綁定 Email');
  await emailjs.send(MAIL.serviceId, MAIL.templateId, { to_email: user.email, to_name: user.nickname || uid || 'SecretRoom Member', status_text: 'SecretRoom 臨時密碼', message: buildRecoveryMessage(code, expiresAtMs), email_type: '帳號安全提醒', member_id: uid || '' }, { publicKey: MAIL.publicKey });
}
...
window.completePasswordResetRequest = async function completePasswordResetRequest(id) {
  ...
  try {
    ...
    if (!uid) throw new Error('這筆申請缺少會員 ID');
    ...
    if (!user.email) throw new Error('這位會員沒有綁定 Email，無法寄出臨時密碼');
    ...
    if (window.showToast) showToast('臨時密碼已寄出，10 分鐘內有效。', 'success');
  } catch (err) {
    console.error(err);
    if (window.showToast) showToast('臨時密碼沒寄成功：' + err.message, 'error');
  }
};
window.rejectPasswordResetRequest = async function rejectPasswordResetRequest(id) {
  try {
    ...
    if (window.showToast) showToast('已拒絕這筆忘記密碼申請。', 'info');
  } catch (err) {
    console.error(err);
    if (window.showToast) showToast('操作失敗：' + err.message, 'error');
  }
};

(() => {
  const VERSION = '20260711-admin-copy-v5';
  ...
  async function refreshAdminMetric() {
    ...
    try {
      ...
      el.title = active.length ? `目前可登入後台：${active.join(', ')}` : '目前沒有可登入的管理員';
      ...
      if (card && !card.querySelector('.sr-admin-count-note')) {
        ...
        note.textContent = '只算 admins 名單，會員資料裡的舊權限不會列入。';
    ...
    btn.onclick = async function(event) {
      ...
      try {
        ...
        if (!snap.exists() || !isExplicitAdmin(snap.data() || {})) {
          ...
          const msg = `這個帳號不在 admins 名單內，無法登入後台：${adminId}`;
  ...
  function improveResetCards() {
    ...
    document.querySelectorAll('#admin-list > div').forEach(card => {
      ...
      note.textContent = t.includes('已處理') ? '這筆已處理，確認會員能登入並完成換密碼即可。' : '處理方式：寄出臨時密碼（10 分鐘有效），會員登入後會先換成自己的密碼。';
  ...
  function enhanceBroadcast() {
    ...
    if (sendBtn && !qs('sr-broadcast-preview-btn')) { const p = document.createElement('button'); p.id = 'sr-broadcast-preview-btn'; p.type = 'button'; p.className = 'mr-2 px-5 py-3 bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-700 rounded-xl text-xs font-bold transition'; p.innerHTML = '<i class="fa-regular fa-eye mr-1.5"></i> 預覽'; p.onclick = () => alert(`通知預覽\n\n標題：${title?.value || '還沒填'}\n\n內容：\n${msg?.value || '還沒填'}`); sendBtn.parentElement.insertBefore(p, sendBtn); }
    if (typeof window.sendPlatformBroadcast === 'function' && !window.sendPlatformBroadcast.__srWrapped) { const original = window.sendPlatformBroadcast; window.sendPlatformBroadcast = async function(...args) { const target = qs('broadcast-target')?.value === 'sg' ? 'S+ . S . G 會員' : '全體會員'; const titleText = qs('broadcast-title')?.value || ''; if (!confirm(`要發給：${target}\n\n標題：${titleText || '還沒填標題'}\n\n確定送出嗎？`)) return; const result = await original.apply(this, args); localStorage.removeItem('sr_broadcast_draft_title'); localStorage.removeItem('sr_broadcast_draft_message'); return result; }; window.sendPlatformBroadcast.__srWrapped = true; }
  }
  function addToolbar() {
    ...
    bar.innerHTML = '<span><i class="fa-solid fa-arrows-rotate text-amber-400 mr-1.5"></i> 資料會即時更新，紅色和橘色項目建議先處理。</span><span>排序建議：最新 → 高風險 → 其他待處理</span>';
